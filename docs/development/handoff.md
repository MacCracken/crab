...
| Build | x86_64 **411,312 B** · `--agnos` **420,040 B** at 0.7.1 · `--win` fails (pre-existing). ⭐ At the 0.7.0 tree a clean-tarball build reproduces **398,504 / 406,992** exactly, and the agnos binary is **byte-identical to the artifact that burned on iron** — so that artifact IS reproducible, contrary to an earlier claim here. |
| Tests | `cyrius test` **408 / 0** · `render_test` **19** checks, 0 failed (it was 14, never 15) · ⭐ **bench now measures the sort**, not `bench_noop` · fuzz still a scaffold that reads none of its input |
| Coverage | **65/89 fns (73 %)**, 6/6 files. ⚠ **DOWN again, and again it is arithmetic rather than rot**: 0.7.1 added 29 functions (the write layer, the shims, the predicates) faster than it added *references* to them — and a `crab_fs_*` called only from `tests/crab.tcyr` still counts as referenced, so the drop is smaller than the work. ⛔ Reference coverage is a floor, not a correctness proof, and the v1.0 criterion of 80 % is now further away than at 0.7.0. |
...
| Mid-arc work | M3 is complete and its five review defects are **closed in 0.7.1**. M4 is **started, not done**: copy/move/delete and the pane states shipped; rename/mkdir are written and tested but not key-bound, and the transfer tray is the one genuinely gated item (dhancha PROGRESS). ⚠ **The kernel has moved past what crab has ever run on** — the 2026-08-30 burn was agnos **1.56.53**, and agnos HEAD is **1.56.55**, which rewrote `is_user_range` from a VA bounds check into a live page-table walk. That validator sits on crab's only kernel-facing hot path. Ordinary ring-3 buffers should pass; **a re-burn is a gate, not a formality**. |
