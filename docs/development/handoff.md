...
| Build | x86_64 **419,816 B** · `--agnos` **437,008 B** at 0.7.3 · `--win` fails (pre-existing). ⭐ At the 0.7.0 tree a clean-tarball build reproduces **398,504 / 406,992** exactly, and the agnos binary is **byte-identical to the artifact that burned on iron** — so that artifact IS reproducible, contrary to an earlier claim here. |
| Tests | `cyrius test` **520 / 0** · `render_test` **26** checks, 0 failed (it was 14, never 15) · ⭐ **bench now measures the sort**, not `bench_noop` · fuzz still a scaffold that reads none of its input |
| Coverage | **88/109 fns (80 %)**, 6/6 files — **the v1.0 criterion, met.** It dipped to 73 % mid-cut and was recovered by writing the assertions genuinely missing (`crab_entry_cmp` directly, the result messages, the notice channel, the listing accessors), not by naming functions to move the number. ⛔ Still reference coverage — a floor, not a correctness proof. The roadmap now gates it per release. |
| Source | **3,897** lines across **six** files. ⚠ Re-derive with `wc -l src/*.cyr`; the per-file numbers that used to sit here went stale at two of the last three cuts, so they are deliberately not repeated. ⚠ Re-derive with `wc -l src/*.cyr`; this row has been stale at two of the last three cuts. |
